...
description: Backend specialist for Cloudflare Workers + Hono API. Use proactively for backend tasks.
...
#    ✅ /path/to/worktree/phase-1-auth/workers/src/routes/auth.ts
```
...
📁 워크트리에서 작업을 시작합니다.
   - 대상 파일: workers/src/routes/auth.ts
   - 테스트: workers/src/__tests__/auth.test.ts
```
...
# 2. 구현 코드 작성
# - workers/src/routes/auth.ts
# - workers/src/services/auth.service.ts 등
...
[ ] npm run test:workers 실행 시 FAILED?

# T*.1/T*.2 (구현) 커밋 전:
[ ] 기존 테스트 파일 존재? (T0.5.x에서 작성됨)
[ ] npm run test:workers 실행 시 PASSED?
[ ] 새 테스트 파일 추가 안 함?
```
...
출력 형식:
- 코드블록 (TypeScript)
- Router 파일 (workers/src/routes/)
- Schemas (contracts/schemas/)
- Handlers (workers/src/handlers/)
- 파일 경로 제안
- 필요한 의존성
...
**완료 조건:** `npm run test:workers` 모두 통과 (🟢 GREEN)
