...
- prompt: |
    ...
    ```bash
    git worktree add ../med-herb-phase1-auth -b phase/1-auth
    cd ../med-herb-phase1-auth
    ```

    ## TDD 요구사항 (Phase 1+ 필수!)
    반드시 TDD 사이클을 따르세요:
    1. RED: 테스트 먼저 작성 (workers/src/__tests__/auth.test.ts)
    2. GREEN: 테스트 통과하는 최소 구현
    3. REFACTOR: 테스트 유지하며 코드 정리

    테스트 명령어: `npm run test:workers`
    ...
    - 완료 보고 형식에 맞춰 보고
    - 사용자 승인 후에만 main 병합
    - 병합 후 worktree 정리: `git worktree remove ../med-herb-phase1-auth`
```
...
| subagent_type | 역할 |
|---------------|------|
| `backend-specialist` | Cloudflare Workers + Hono, API 엔드포인트, D1 접근 |
| `frontend-specialist` | React + Vite UI, Zustand 상태관리, API 통합 |
